...
export default function BookCard(props: Book) {
  ...
  // (Device Width / Card Amount) - (X Padding - Gutter - (Card Border Width * 2))
  const cardWidth = Dimensions.get("screen").width / 2 - (48 - 16 - 4);
  // cardWidth - Aspect Ratio
  const cardHeight = cardWidth * (136 / 87);

  return (
    <Pressable onPress={onShortPress} onLongPress={onLongPress}>
      <View
        className={cn(
          "border border-slate-300 rounded-[14px] bg-slate-50 overflow-hidden justify-end",
          isSelected ? "border-pink-600" : "border-none"
        )}
      ...
      >
        <View className="p-4 relative z-20">
          <View className="gap-1">
            <Text
              className="font-outfit text-base leading-5 text-slate-50"
              numberOfLines={2}
            >
              {props.title}
            </Text>
